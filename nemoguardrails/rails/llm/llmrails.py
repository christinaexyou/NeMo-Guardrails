...
class LLMRails:
    ...
    def __init__(
        ...
        self.model_factory = ModelFactory(config, self.llm)
        self.llm, llms, model_caches = self.model_factory._init_llms()
        ...
        # Copy model attributes from model factory to self
        for model_type in llms:
            attr_name = f"{model_type}_llm"
            if hasattr(self.model_factory, attr_name):
                setattr(self, attr_name, getattr(self.model_factory, attr_name))
                self.runtime.register_action_param(attr_name, getattr(self, attr_name))
    ...
    def stream_async(
        ...
        self.model_factory._configure_main_llm_streaming(self.llm)
